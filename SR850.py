...
"""
This code is modified from SR830 code in Qcodes library. This part only support lock-in detection.
There might not be any support on output-related adjustment(eg. adjustable gain etc.).
"""
...
class ChannelBuffer(ArrayParameter):
    ...
    !!!This class is originally made for SR830, which has not been tested for SR850.
    Although it might have a chance to work. If SR850 and SR830 shares the same logic
    and most of the TSP command
    """
    ...
    def __init__(self, name: str, instrument: 'SR850', channel: int) -> None:
        ...
        Args:
            ...
            instrument (SR850): The parent instrument
    ...
    def get_raw(self):
        ...
        if self.shape[0] != N:
            raise RuntimeError("SR850 got {} points in buffer expected {}".format(N, self.shape[0]))
        return numbers


class SR850(VisaInstrument):
    """
    This is the qcodes driver for the Stanford Research Systems SR850
    Lock-in Amplifier
    """
    """
    The following voltage/current range values and settings are verified with SR850
    """
    ...
    _INPUT_CURRENT_CONFIG_GAIN_TO_N = {
        '1 M': 0,
        '100 M': 1,
    }
    _N_TO_INPUT_CURRENT_GAIN_CONFIG = {v: k for k, v in _INPUT_CURRENT_CONFIG_GAIN_TO_N.items()}
    ...
    def __init__(self, name, address, **kwargs):
        ...
        #added for sr850
        self.add_parameter('input_current_gain_config',
                           label='Input current gain configuration',
                           get_cmd='IGAN?',
                           get_parser=self._get_input_current_gain_config,
                           set_cmd='IGAN {}',
                           set_parser=self._set_input_current_gain_config,
                           vals=Enum(*self._INPUT_CURRENT_CONFIG_GAIN_TO_N.keys()))
    ...
    # AUX for snap is commented for SR850
    SNAP_PARAMETERS = {
            'x': '1',   
            'y': '2',  
            'r': '3', 
    ...
    def _set_buffer_SR(self, SR):
        ...
        self._buffer2_ready = False
    """
    The channel definition of SR850 is different from SR830. So the next two def might not work
    ...
    #SR 850 does not have DDEF command.
    ...
    _N_TO_INPUT_CURRENT_GAIN_CONFIG = {v: k for k, v in _INPUT_CURRENT_GAIN_CONFIG_TO_N.items()}
    """
    def _get_input_current_gain_config(self, s):
        mode = self._N_TO_INPUT_CURRENT_GAIN_CONFIG[int(s)]

        if mode in ['1 M', '100 M']:
            self.sensitivity.vals = self._CURR_ENUM
            self._set_units('A')
        else:
            self.sensitivity.vals = self._CURR_ENUM
    ...
    def _set_input_current_gain_config(self, s):
        if s in ['1 M', '100 M']:
            self.sensitivity.vals = self._CURR_ENUM
            self._set_units('A')
        else:
        ...
        return self._INPUT_CURRENT_CONFIG_GAIN_TO_N[s]
